...
async def test_ollama_adapter_generation_success():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        assert result == "Epistemology is the study of knowledge."
        mock_post.assert_awaited_once()

@pytest.mark.asyncio
async def test_ollama_adapter_error_handling():
    adapter = OllamaLocalAdapter(model_name="test-llama", base_url="http://localhost:11434", timeout_seconds=1.0)
